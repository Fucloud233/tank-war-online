...
public class Tank extends Entity {
    ...
    // 记录坦克当前的子弹数量
    private int bulletNum = GameConfig.TankMaxBulletNum;


    // tank构造函数(随机方向)
    // 不能直接删除
    public Tank(int id, double x, double y) {
        super(GameConfig.TankWidth, GameConfig.TankHeight, x, y);
        this.id = id;
        // 随机坦克方向
    ...
    public void setDirection(Direction dir) {
        ...
        switch (dir) {
            case LEFT, RIGHT -> {
                this.width = GameConfig.TankWidth;
                this.height = GameConfig.TankHeight;
            }
            case UP, DOWN -> {
                this.width = GameConfig.TankHeight;
                this.height = GameConfig.TankWidth;
            }
    ...
    public void move() {
        ...
        // 根据当前方向和速度移动
        int speed = GameConfig.TankSpeed;
        switch (dir) {
            case LEFT -> x -= speed;
            case RIGHT -> x += speed;
            case UP -> y -= speed;
    ...
    public Bullet fire() {
        ...
        // 枪口到子弹中心点位置
        double distance = + GameConfig.TankHeight / 2 + GameConfig.BulletSize / 2;
        switch (this.dir) {
    ...
    public void recoveryBullet() {
        if(bulletNum < GameConfig.TankMaxBulletNum)
            this.bulletNum ++;
    }
    ...
    public boolean isCollidingWith(Entity entity) {
        if (entity.isAlive()) {
            ...
            // 将坦克尾部从碰撞域中减去
            float delta = (GameConfig.TankWidth - GameConfig.TankHeight) / 2;
            switch (this.dir) {
                case UP ->
...
class TankImg {
    ...
    private static void loadImages(Direction dir) {
        Image[] images = new Image[GameConfig.MaxPlayerNumber];
